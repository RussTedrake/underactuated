...
np.testing.assert_allclose(autodiff.derivatives(), [-0.620764, -0.389539], atol=1e-3)
print()
...
def finite_difference_check_autodiffs(autodiff_params=False, debug=False):
    ...
    for kNetConstructor in NETS_TO_TEST:
        ...
        in_vec = np.array([AutoDiffXd(values[i], one_hot(i, total_params)) for i in range(n_inputs)])
        if autodiff_params:
            values = np.hstack(param.clone().detach().numpy().flatten() for param in network.parameters())
            param_vec = np.array([AutoDiffXd(values[i], one_hot(n_inputs+i, total_params)) for i in range(n_params)])
