...
for kNetConstructor in [FC, FCBIG, MLPSMALL, MLP]:
    num_params = sum(tensor.nelement()
                     for tensor in kNetConstructor().parameters())
    total_params = sum((num_inputs, num_states, num_params))
    print("total params: ", total_params, end='\t')
    sys.stdout.flush()

    constraint = make_NN_constraint(kNetConstructor,
                                    num_inputs,
                                    num_states,
                                    num_params)
    ...
    # Set derivatives array to length total_param with only index i set for ith
    # AutoDiff.  values = np.random.randn(total_params)
    ...
    print("ad pass")
    uxT = np.array([AutoDiffXd(values[i], one_hot(i, total_params))
