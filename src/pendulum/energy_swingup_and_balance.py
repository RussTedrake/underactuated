...
def TotalEnergy(state, params):
    theta = state[0]
    thetadot = state[1]
    total_energy = .5 * params.mass() * math.pow(params.length() * thetadot, 2) - \
...
def PhasePlot(pendulum):
    ...
    params = PendulumParams() # = pendulum.CreateDefaultContext().get_numeric_parameter(0)
    ...
    for e in E:
        for i in range(theta.size):
            v = (e + params.mass()*params.gravity()*params.length()*math.cos(theta[i]))/(.5* params.mass() * params.length()*params.length())
            if (v>=0):
...
#    simulator.set_publish_every_time_step(False)
    ...
    for i in range(10):
        ...
        context.set_time(0.)
        state.SetFromVector(UprightState().CopyToVector() + math.pi*np.random.randn(2,))
        simulator.StepTo(4.)
