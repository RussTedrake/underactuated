...
import numpy as np
from pydrake.all import (DiagramBuilder, SceneGraph, Simulator, LeafSystem, DiagramBuilder, SceneGraph,
                         ZeroOrderHold)
from pydrake.examples.pendulum import PendulumGeometry, PendulumPlant


class TestFittedCartpole(unittest.TestCase):
    ...
    def test_state_cost(self):
        ...
        self.assertLessEqual(
            ...
            tol,
            msg=f'target state cost is {compute_state_cost(Q, target_state, target_state)} which is more than tolerance {tol}')

        self.assertLessEqual(
            np.abs(compute_state_cost(Q, target_state, np.array([0,0]).reshape(-1,1))- np.pi**2*20.),
            tol,
            msg='Cost at state = 0 is incorrect')

        test_states = np.array([[  5.89375203,   5.72132364,   2.07924145, -25.58778621, 4.84281921, -19.20542969,  -2.35612669],
                    [-35.69553553,  24.11618679,  -0.85857409,   3.61505439, -3.28412933,  26.96593774,  -0.81457745]])
        result_true = np.array([30201.18682396,  8831.40552794,   324.34992454,   659.21812345, 849.25095811, 11720.8369055 ,   318.57697075])
        result_tested = compute_state_cost(Q, target_state, test_states)
        for i in range(result_true.shape[0]):
            self.assertLessEqual(np.abs(result_true[i]-result_tested[i]), tol, f"failed on random state number {i}")
    ...
    def test_compute_u_star(self):
        ...
        dJdX = np.array([[ 8.455e+00, -2.330e+00,  1.650e-01,  2.040e+00, -3.945e+00],
        ...
        u_star_true = np.array([[  2.20789167,   2.98984167,  15.70045   , -12.72798333, 7.183475]])
        u_star_test = compute_u_star(R_diag, dJdX, dstate_dynamics_du)
        self.assertTrue(u_star_true.shape == u_star_test.shape, f"output of compute_u_star is incorrect. Expected {u_star_true.shape} got {u_star_test.shape}")

        for i in range(u_star_test.shape[0]):
            for j in range(u_star_test.shape[1]):
                self.assertLessEqual(np.abs(u_star_test[i,j]-u_star_true[i,j]), 1e-8, f"incorrect u_star at index {i} for test state {j}")
    ...
    def test_policy(self):
        ...
        compute_u_star = self.notebook_locals['compute_u_star']
        simulator = self.build_pendulum_simulator(value_mlp, value_mlp_context, R_diag, compute_u_star)
        ...
        for i in range(num_sim):
            ...
            final_state_vector = simulator_context.get_continuous_state().get_vector().CopyToVector()
        ...
        self.assertGreaterEqual(num_passed, threshold, f"Only passed {num_passed/num_sim * 100}% of initial configurations. Need to pass {frac*100}%")

    # initialize controller and plant
    def build_pendulum_simulator(self, value_mlp, value_mlp_context, R_diag, compute_u_star):
        time_step = 0.01
