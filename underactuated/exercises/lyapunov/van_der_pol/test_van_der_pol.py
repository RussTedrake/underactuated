...
class TestVanDerPol(unittest.TestCase):
    ...
    def test_method_1(self):
        ...
        self.assertTrue(is_verified(rho_method_1),
            "Could not verify the level set rho_method_1 with the function is_verified(rho)."
            )
        rho_step = self.notebook_locals['rho_step']
        self.assertAlmostEqual(rho_step, 0.01,
            msg="rho_step is different from the predefined value of 0.01."
            )
        self.assertFalse(is_verified(rho_method_1 + rho_step),
            "The function is_verified(rho) could verify the level set rho_method_1 + rho_step: line search is not complete."
            )
    ...
    def test_method_2(self):
        ...
        rho = variables[0]
        result = Solve(prog)
        self.assertAlmostEqual(result.GetSolution(rho), rho_method_2,
    ...
    def test_method_3(self):
        ...
        rho = variables[0]
        result = Solve(prog)
        self.assertAlmostEqual(result.GetSolution(rho), rho_method_3,
