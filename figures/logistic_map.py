...
from pydrake.all import (DiagramBuilder, LogOutput, Simulator, SymbolicVectorSystem, Variable)


builder = DiagramBuilder()

x = Variable("x")
plant = builder.AddSystem(SymbolicVectorSystem(state=[x], dynamics=[4*x*(1-x)], output=[x], time_period=1))
...
p0 = 1 + 0*x
p1 = 1./(2*np.sqrt(1-x))
p2 = np.sqrt(2)/(8*np.sqrt(1-x))*(1./np.sqrt(1+np.sqrt(1-x)) + 1./np.sqrt(1-np.sqrt(1-x)))
