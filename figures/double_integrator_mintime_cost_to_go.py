...
for i in range(len(q)):
    for j in range(len(qdot)):
        if (qdot[j] < 0 and q[i] < 0.5 * qdot[j] ** 2) or (
            qdot[j] >= 0 and q[i] < -0.5 * qdot[j] ** 2
        ):
            J[i, j] = 2 * np.sqrt(0.5 * qdot[j] ** 2 - q[i]) - qdot[j]
        else:
            J[i, j] = qdot[j] + 2 * np.sqrt(0.5 * qdot[j] ** 2 + q[i])
...
fig.update_layout(
    scene={"xaxis_title": "q", "yaxis_title": "q̇", "zaxis_title": "J"},
